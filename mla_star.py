...
def sane_starts_tasks(starts, tasks, nav_space, num_agents):
    ...
    for task in tasks:
        assert len(task) == 2
        for i in range(2):
            assert task[i][0] >= 0 and task[i][0] < nav_space.shape[1], '{} {}'.format(task[i][0], nav_space.shape[1])
            assert task[i][1] >= 0 and task[i][1] < nav_space.shape[0], '{} {}'.format(task[i][1], nav_space.shape[0])
...
if __name__ == '__main__':
    ...
    sane_starts_tasks(starts, tasks, nav_space)
